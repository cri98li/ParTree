...
def arya_etal_k_median(df, n_clusters, num_trial = 1):
    ...
    for c in best_cluster_centers:
        actual_centers.append(df.iloc[c].tolist())
...
def vanilla_clustering(df, num_clusters, clustering_method,seedValue):
    if clustering_method == "kmeans":
        kmeans = KMeans(num_clusters,random_state=seedValue)
        kmeans.fit(df)
        initial_score = -kmeans.score(df)
        pred = kmeans.predict(df)
        print("Vanilla Score is ",str(initial_score))
