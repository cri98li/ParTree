...
def fair_clustering(dataset, config_file, seedValue,data_dir, num_clusters, deltas, max_points, violating, violation):
    ...
    ty = df['type']
    c1 = len(list(np.where(ty==0))[0])
    c2 = len(list(np.where(ty==1))[0])
    print(c1)
    print(c2)
    ...
    if not violating:
        t1 = time.monotonic()
        initial_score, pred, cluster_centers = vanilla_clustering(df, num_clusters, clustering_method,seedValue)
        t2 = time.monotonic()
        cluster_time = t2 - t1
    ...
    for delta in deltas:
        ...
        output["alpha"] = alpha
        output["beta"] = beta
        output["Total Runtime"] = time_elapsed
        print("Total Runtime "+str(time_elapsed))
