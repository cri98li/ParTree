...
class PrincipalParTree(ParTree):
    ...
    def _compute_penalty(self, points, labels, distances, indices):
        n = len(points)
        penalties = 0
        count_comp = 0
        unique_labels = np.unique(labels)
        cluster_indices = {label: np.where(labels == label)[0] for label in unique_labels}
        #print("prima calcolo ind")
        #tree = KDTree(points)
        #distances = pairwise_distances(points)
        print("labels pre penalty ind", list(labels))
        penalty_ind = _fairness_ind(n, points, labels, distances, indices)
        print("after penalty ind", penalty_ind)
        #penalty_ind = 0
        #print("prima calcolo dem")
        penalty_dem = _fairness_dem(points, labels, cluster_indices, self.protected_attribute)
        print("dopo calcolo dem", penalty_dem)
        penalty_gro = _fairness_gro(points, labels, self.protected_attribute)

        #print("prima calcolo alfa")
    ...
    def _make_split(self, idx_iter):
        ...
        print("n_components_split after computation", n_components_split)
        ...
        print("self.X[idx_iter]", self.X[idx_iter])
        print("typed_X", typed_X)
        ...
        r2_c_list = list()
        print("corrected")
        #print("trange", trange(n_components_split, disable=not self.verbose))
        for i in trange(n_components_split, disable=not self.verbose):
            print("i of the trange", i)
            for feature_index in range(self.X.shape[1]):
                print("******************feature**************+****", feature_index, range(self.X.shape[1]), self.X.shape[1])
                ...
                tree = KDTree(self.X[idx_iter])
                distances, indices = tree.query(self.X[idx_iter], k=6)
                #distances = pairwise_distances(self.X[idx_iter])
                #print("range(len(thresholds) - 1)", range(len(thresholds) - 1))
                for idx_threshold in range(len(thresholds) - 1):
                    #print("dentro idx threshold loop")
                    ...
                    #print("after modified_x stuff")
                    clf_i.fit(modified_X[idx_iter], y_pca)
                    print("y_pca", y_pca)
                    print("modified_X[idx_iter]", modified_X[idx_iter])
                    labels_i = clf_i.apply(modified_X[idx_iter])
                    idx_l = np.where(labels_i == 1)[0]
                    print("idx_l", idx_l)
                    idx_r = np.where(labels_i == 2)[0]
                    print("idx_l", idx_r)
                    if len(idx_l) == 0 or len(idx_r) == 0:
                        continue
                    ratio_threshold = 0.1  # or another suitable ratio
                    if min(len(idx_l), len(idx_r)) / max(len(idx_l), len(idx_r)) < ratio_threshold:
                        continue
                    print("labels_i prima check di 0", labels_i)
                    if np.any(labels_i == 0):  # Skip if there are any zeros in labels_i
                        continue
                    print("dopo il fit labels_i", labels_i)
                    temp_score = clf_i.score(self.X[idx_iter], y_pca)
                    print("dopo calcolo temp score", temp_score)
                    bic_children_i = bic(self.X[idx_iter], (np.array(labels_i) - 1).tolist())
                    #print("dopo calcolo bic e prima della penalty")

                    alfa = self._compute_penalty(self.X[idx_iter], labels_i, distances, indices)
                    #else:
                    #    penalty = 0
                    #print("prima calcolo composite score")
                    composite_score = self.sigmoid(temp_score) - alfa #*abs(temp_score-1)
                    print("alfa dopo calcolo composite score", alfa)
                    new_temp_score = self.sigmoid(temp_score)
                    ...
                    if composite_score > best_split_score:
                        ...
                        best_r2_score = new_temp_score
                        best_alfa = alfa
                        best_clf = clf_i
                        best_labels = labels_i

                    #self._write_to_file(f"\tFeature {feature_index}, Value {value}, Iteration Final Split Score {composite_score}, Penalty {best_alfa}, R2 Score {new_temp_score}, BIC {bic_children_i}")
                ...
                for index, value in enumerate(labels_i):
                    if value == 1:
                        ...
                        #print("indexes_of_2", indexes_of_2)

                protected_attribute_arr = self.X[:, self.protected_attribute]
                #print("self.X", self.X)
                print("protected_attribute_arr", protected_attribute_arr)
                    #print("indexes_of_1",indexes_of_1)
               ...
               #     similar_count_log += similar_count
                ...
                #self._write_to_file(f"\nBest split for feature {feature_index}: Value {best_split_value}, Best Split Score {best_split_score}, R2 Score {best_r2_score}, Penalty {best_alfa}, BIC {best_bic_score}, Cluster 1 {cluster_1}, Cluster 2 {cluster_2}")
        ...
        print("labels before returning", labels)
