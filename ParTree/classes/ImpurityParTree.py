...
def _make_split_innerloop(X, criteria_clf, criteria_reg, agg_fun, is_categorical_feature, idx_iter, feature, feature_values):
    best_feature = None
    best_threshold = None
    best_impurity = np.inf
    ...
    for threshold in feature_values[feature]:
        ...
        for target_feature in range(n_features):
            ...
            if is_categorical_feature[target_feature]:
                ...
                mean_val_a = np.array(
                    [np.mean(X_a[:, target_feature])] * len(X_a)
                )
                mean_val_b = np.array(
                    [np.mean(X_b[:, target_feature])] * len(X_b)
                )
                # print(X_a[:, target_feature].shape, mean_val_a.shape)
                imp_a = criteria(X_a[:, target_feature], mean_val_a)
                imp_b = criteria(X_b[:, target_feature], mean_val_b)

            impurity = (
                    len(X_a) / len(X[idx_iter]) * imp_a
                    + len(X_b) / len(X[idx_iter]) * imp_b
            )
            impurity_list.append(impurity)

        impurity_agg = agg_fun(impurity_list)
        # print(feature, threshold, impurity_agg, best_impurity, impurity_list)
